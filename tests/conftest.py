...
def mock_hub(request: pytest.FixtureRequest) -> Generator[MagicMock]:
    ...
    with patch(
        ...
        def create_mock_hub(*args, **kwargs):
            ...
            client.read_holding_registers = AsyncMock(side_effect=lambda *args, **kwargs: create_mock_response())
            client.read_input_registers = AsyncMock(side_effect=lambda *args, **kwargs: create_mock_response())
            client.read_discrete_inputs = AsyncMock(side_effect=lambda *args, **kwargs: create_mock_response(bits=[False]*50))
            client.read_coils = AsyncMock(side_effect=lambda *args, **kwargs: create_mock_response(bits=[False]*50))
...
def mock_modbus_client():
    ...
    with patch(
        "homeassistant.components.qube_heatpump.hub.AsyncModbusTcpClient",
    ) as mock_client:
