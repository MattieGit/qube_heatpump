...
- chore: Repo hygiene: the stray copy of Home Assistant core's `pyproject.toml` replaced by this repo's own tooling config; `template_sensors.yaml` and planning documents removed; CI tests against Home Assistant 2026.9 on Python 3.14 (pinned test framework; was HA 2025.1) and runs ruff; test suite rebuilt on a stateful fake device with per-platform snapshots (324 tests, ~1,500 lines of pasted mocks and vacuous assertions removed).
...
## 2026.8.1 — 2026-08-05
- fix: Virtual thermostat resumes heating after a temperature-sensor timeout; previously demand stayed off until an unrelated too-warm reading.
- fix: `write_register` and `reconfigure` services survive reloads and raise proper errors; the non-functional `data_type` field was removed. Tariff/SCOP day and month sensors no longer depend on a broken deduplication token; DNS resolution no longer runs on every poll.
- chore: Code-quality release (#31): ~1,900 lines of dead code removed, shared base entity, entity and unique IDs pinned by a snapshot test, new thermostat/DHW scheduler test coverage, diagnostics download extended. No entity ID changes.

## 2026.7.1 — 2026-07-06
- feat: Fetch all values via batched block reads (about 7 Modbus transactions per poll instead of 94; requires python-qube-heatpump >= 1.12.0). Fixes the intermittent "No response received after 3 retries" errors seen after 2026.6.2.
- fix: Transient read failures are logged once as a warning and then at debug level; a rejected block read falls back to individual register reads.

## 2026.6.2 — 2026-06-15
- fix: Show firmware version "0.00" instead of unknown when register 77 reads 0.0 (the register does not match the firmware shown on the display; reported to HR Energy).

## 2026.6.1 — 2026-06-15
- fix: Remove the unused Dutch `object_base` strings from the computed valve and status sensors; entity IDs generated by the code are English. Requires python-qube-heatpump >= 1.11.1.

## 2026.4.1 — 2026-04-13
- feat: Unify anti-legionella into `sensor.qube_status_heatpump` (#29): the status shows "Anti-legionella" while a cycle runs and overrides every state except Alarm; `binary_sensor.qube_req_antileg_1` stays as a diagnostic trigger. Requires python-qube-heatpump >= 1.9.0.
- refactor: Delegate monotonic clamping to the library.
- docs: Update the wiki with community feedback.

## 2026.3.4 — 2026-03-18
- feat: Device info shows the heat pump firmware version read from register 77 instead of the integration version; the info sensor splits `version` into `firmware_version` and `integration_version`. Requires python-qube-heatpump >= 1.6.0 (auto-reconnect with backoff).
- fix: Coordinator overrides `_async_update_data` instead of passing `update_method`.

## 2026.3.3 — 2026-03-06
- fix: Require python-qube-heatpump >= 1.5.4, which reverts the compressor speed unit from RPS back to RPM.

## 2026.3.2 — 2026-03-06
- fix: Require python-qube-heatpump >= 1.5.3 so the compressor speed is displayed as an integer.

## 2026.3.1 — 2026-03-06
- change: Scan interval 10 s → 15 s.
- fix: Apply core review feedback: remove the reserved "unknown" state from the status sensor (unmapped codes return None), rename the flow sensors to "Measured PVT flow", "Compressor start up" → "Compressor startup". Requires python-qube-heatpump >= 1.5.2.

## 2026.2.5 — 2026-02-23
- docs: Update the Dutch valve entity IDs to English in the example dashboard and the wiki.

## 2026.2.4 — 2026-02-12
- fix: Persist the monotonic-clamping cache to disk (`.storage/qube_heatpump_monotonic_<entry_id>`, saved every 5 minutes and restored before the first refresh) and keep it on the coordinator instead of a per-cycle dict, so `total_increasing` sensors no longer report a tiny decrease after a Home Assistant restart (#27).
